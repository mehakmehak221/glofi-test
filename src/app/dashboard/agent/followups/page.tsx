...
export default function AgentFollowUpsPage() {
  ...
  const TAB_FILTERS = [
    { label: `All Follow-ups (${total})`, value: '' as FollowUpStatus | '' },
    { label: 'Pending', value: FollowUpStatus.PENDING },
    { label: 'Completed', value: FollowUpStatus.COMPLETED },
    ...
    <div className="min-h-screen p-4 sm:p-6 lg:p-8 max-w-[1200px] mx-auto font-montserrat space-y-6">
      ...
      <div>
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-xs text-[var(--sidebar-text)] opacity-60">• Follow-Up Queue</span>
        </div>
        <h1 className="text-3xl font-bold text-[var(--foreground)] tracking-tight">
          Scheduled Follow-ups
        </h1>
        <p className="text-sm text-[var(--sidebar-text)] opacity-60 mt-1">
          Stay on top of your client communications, call schedules, and KYC reminders.
        </p>
      </div>
      ...
      {isLoading ? (
        <div className="bg-[var(--card-surface)] border border-[var(--sidebar-border)] rounded-md p-8 text-center">
          ...
          <p className="text-sm text-[var(--sidebar-text)] opacity-60">Loading follow-ups queue...</p>
        </div>
      ) : followUps.length === 0 ? (
        <div className="bg-[var(--card-surface)] border border-[var(--sidebar-border)] rounded-md p-12 text-center">
          ...
          <h4 className="text-base font-bold text-[var(--foreground)]">No Follow-ups Found</h4>
          <p className="text-xs text-[var(--sidebar-text)] opacity-60 mt-1">You have no scheduled follow-ups matching this view.</p>
        </div>
      ) : (
              ...
              >
                ...
                <div className="flex items-center justify-between pt-4 mt-4 border-t border-[var(--sidebar-border)] text-[11px] text-[var(--sidebar-text)] opacity-50">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span>Due: {new Date(fu.dueAt).toLocaleString()}</span>
                  </div>
                </div>
              </div>
            );
