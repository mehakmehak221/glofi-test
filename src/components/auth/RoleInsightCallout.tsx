...
export default function RoleInsightCallout({ role }: { role: string }) {
    ...
    return (
        <div className="flex gap-3 p-4 rounded-xl border border-[var(--color-border-subtle)] bg-[var(--color-bg-surface-subtle)]/90">
            <SparkleCluster className="shrink-0 mt-0.5" />
            <p className="text-sm text-white leading-snug font-montserrat">{text}</p>
        </div>
    );
}
